...
我们假设有一系列agent，它们分别可以执行不同数量的任务量，同时需要消耗不同数量的资源。
...
在分配时，我们将它们按照产出效率（完成工作份数/消耗资源数）进行排序，优先将资源分配给产出效率高的agent。
...
```powershell
Allocated agents: [(2, 1, 2), (3, 4, 9), (4, 3, 7)]
...
$$x1 = x+8\cos{\theta}\quad y1=y+8\sin{\theta}$$
$$x2 = x+2\sin{\theta}\quad y2 = y-2\cos{\theta}$$
$$x3 = x-2\sin{\theta}\quad y3=y+2\cos{\theta}$$
